function [ECG_i]=fixed_pan_tompkins(ECG,fs)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%BANDPASS FILTER
fl = 5*(2/fs); fh = 15*(2/fs); %cut off frequencies of the filter
N = 3; %order fo the filter
Wn = [fl fh]; %normalized frequency with respect to sampling frequency 
b2=[5.622892814127881e-04 0 -0.001686867844238 0 0.001686867844238 0 -5.622892814127881e-04];
a2 = [1 -5.586769084540233 13.072610195820271 -16.399515633390067 11.633482474319420 -4.424829744802687 0.705031824499085];
[ECG_b2 z1] = filter(b2,a2,ECG);
ECG_b  = filter(b2,a2,ECG,z1);

%figure(1),
%subplot(2,1,1), plot(ECG_b), axis tight
%subplot(2,1,2), plot(ECG_b2), axis tight
